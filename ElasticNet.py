...
def Poly_lasso(degree,alpha):
    return Pipeline([
    ("poly",PolynomialFeatures(degree=degree)),
    ("dt_scalar",StandardScaler()),
    ("lasso_reg",Lasso(alpha=alpha)) #这里使用的线性回归
])
...
#使用岭回归(正则化) 让曲线更加的平缓
poly_rige = Poly_rige(degree=20,alpha=0.001)
poly_rige.fit(x_train,y_train)
p = poly_rige.predict(x_test)
print(mean_squared_error(y_test,p))
#plot_model(poly_rige)

#使用lasso回归(正则化) 让曲线更加的平缓 (重要：特征选择)
poly_lasso = Poly_lasso(degree=20,alpha=0.1)
poly_lasso.fit(x_train,y_train)
p = poly_lasso.predict(x_test)
print(mean_squared_error(y_test,p))
plot_model(poly_lasso)